...
public class VirtualProxyExpressionDecorator extends VariableExpressionBuilderDecorator {
    ...
    @Override
    public TypedExpression buildVariableExpression(InjectionBuilderContext context, InjectionNode injectionNode) {
        VirtualProxyAspect proxyAspect = injectionNode.getAspect(VirtualProxyAspect.class);
        Map<InjectionNode, TypedExpression> variableMap = context.getVariableMap();
        TypedExpression variable;

        if (proxyAspect != null && proxyAspect.isProxyRequired() && !proxyAspect.isProxyDefined()) {
            //proxy
            JClass proxy = virtualProxyGenerator.generateProxy(injectionNode);
            JExpression proxyExpression = proxyVariableBuilder.buildProxyInstance(context, injectionNode, proxy);
            ...
            context.getProxyLoad().put(injectionNode, variable);
        } else {
            variable = getDecorated().buildVariableExpression(context, injectionNode);
        }
